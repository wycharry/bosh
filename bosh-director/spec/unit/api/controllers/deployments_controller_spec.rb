    ...
    describe Controllers::DeploymentsController do
      ...
      describe 'API calls' do
        shared_examples_for 'a deployment' do
          context 'authenticated access' do
            it 'expects compressed deployment file' do
              post url, spec_asset('test_conf.yaml'), {'CONTENT_TYPE' => 'text/yaml'}
              expect_redirect_to_queued_task(last_response)
            end

            it 'only consumes text/yaml' do
              post url, spec_asset('test_conf.yaml'), {'CONTENT_TYPE' => 'text/plain'}
              expect(last_response.status).to eq(404)
            end

            it 'gives a nice error when request body is not a valid yml' do
              post url, "}}}i'm not really yaml, hah!", {'CONTENT_TYPE' => 'text/yaml'}
            ...
            it 'gives a nice error when request body is empty' do
              post url, '', {'CONTENT_TYPE' => 'text/yaml'}
            ...
            context 'when provided a cloud config and runtime config context to work within' do
              it 'should use the provided context instead of using the latest runtime and cloud config' do
                ...
                deployment_context = ['context', JSON.dump({'cloud_config_id' => 1, 'runtime_config_id' => 2})]
                ...
                post "#{url}?#{URI.encode_www_form(deployment_context)}", spec_asset('test_conf.yaml'), {'CONTENT_TYPE' => 'text/yaml'}
        ...
        context 'creating a deployment' do
          ...
          it_should_behave_like 'a deployment'
        end

        context 'doing a dry run for create deployment' do
          let(:url) { '/dry-run' }
