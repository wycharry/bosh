module Bosh::Director
  ...
  class DeploymentPlan::Assembler
    ...
    def bind_models(skip_links_binding = false)
      ...
      instance_group_migrator = Bosh::Director::DeploymentPlan::InstanceGroupMigrator.new(@deployment_plan, @logger)

      desired_instance_groups.each do |desired_instance_group|
        desired_instances = desired_instance_group.desired_instances
        existing_instances = instance_group_migrator.find_existing_instances(desired_instance_group)
        instance_plans = instance_planner.plan_instances(desired_instance_group, desired_instances, existing_instances)
        desired_instance_group.add_instance_plans(instance_plans)
      end

      instance_plans_for_obsolete_jobs = instance_planner.plan_obsolete_instances(desired_instance_groups, @deployment_plan.existing_instances)
